...
def model_load(target, epochNum):
    ...
    if epochNum == 1:
        ...
        model = keras.models.load_model("models/" + target + '/epoch' + str(epochNum - 1) + '.h5')
    return model
...
def model_save(model, target, epochNum):
    if not os.path.isdir("models/" + target):
        os.mkdir('models/' + target)
    model.save("models/" + target + '/epoch' + str(epochNum) + '.h5')
...
def main(args):
    ...
    data = tensorflow.data.Dataset.from_generator(dataGen, (tf.float32, tf.int32),
                                                  (tf.TensorShape([299, 299, 3]), tf.TensorShape([])))
